...
import { getLastCachedCandle } from "./candleService.js";
...
async function runScanner(scanner: Scanner): Promise<void> {
  ...
  try {
    ...
    const allData: SymbolData[] = activeSymbols.map((sym) => {
      ...
      const price = sym.lastClose ?? inMemory?.close ?? null;
      const volume = sym.lastVolume ?? inMemory?.volume ?? null;
      const open = sym.lastOpen ?? inMemory?.open ?? null;
      const high = sym.lastHigh ?? inMemory?.high ?? null;
      const low = sym.lastLow ?? inMemory?.low ?? null;
      return {
        symbol: sym.symbol,
